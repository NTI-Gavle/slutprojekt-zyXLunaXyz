...
$post = getPostById($dbconn, $postId, (int) $_SESSION['user_id']);

if (!$post) {
    redirect('index.php');
}

$replies = getRepliesByPostId($dbconn, $postId);
    ...
    <main class="min-h-screen lg:ml-72 xl:mr-96 border-x border-neutral-800">
        <section class="sticky top-0 z-20 bg-black/80 backdrop-blur border-b border-neutral-800 px-5 py-4">
            <div class="flex items-center gap-4">
                <a href="index.php" class="text-2xl hover:text-sky-400 transition">
                    ←
                </a>

                <div>
                    <h1 class="text-xl font-bold">Post</h1>
                    <p class="text-sm text-neutral-500">@<?= e($post['username']) ?></p>
                </div>
            </div>
        </section>

        <article class="border-b border-neutral-800 px-4 py-5">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <div class="flex items-center gap-2">
                            <?php if (!$isOwnPost): ?>
                                <form action="follow_user.php" method="post" class="shrink-0">
                                    <input type="hidden" name="user_id" value="<?= (int) $post['user_id'] ?>">
                                    <input type="hidden" name="redirect_to" value="post.php?id=<?= (int) $post['id'] ?>">
                    ...
                    <p class="mt-4 text-xl whitespace-pre-wrap break-words text-left"><?= e($post['content']) ?></p>

                    <?php if (!empty($post['image_url'])): ?>
                        <img
                            src="<?= e($post['image_url']) ?>"
                            alt="Post image"
                            class="mt-4 rounded-2xl border border-neutral-800 max-h-[550px] object-cover"
                        >
                    <?php endif; ?>
            ...
            <form action="create_reply.php" method="post">
                ...
                <div class="flex gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex justify-end border-t border-neutral-900 pt-3">
                            <button
                                type="submit"
        ...
        <section>
            <?php if (empty($replies)): ?>
                <div class="px-6 py-12 text-center text-neutral-500">
                    <h2 class="text-xl font-bold text-white mb-2">No replies yet</h2>
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-1.5">
                                <a
                                    ...
                                    class="font-bold leading-tight hover:underline"
                                >
                                    <?= e($reply['display_name']) ?>
                                </a>

                                <?php if (($reply['role'] ?? '') === 'owner'): ?>
                                    <span class="z-owner-rank">Owner</span>
                                <?php elseif (($reply['role'] ?? '') === 'admin'): ?>
                                    <span class="z-admin-rank">Admin</span>
                                <?php endif; ?>

                                <span class="text-neutral-500">·</span>
                                <span class="text-neutral-500 text-sm"><?= e(date('M j', strtotime($reply['created_at']))) ?></span>
                            </div>
                            ...
                            <p class="mt-2 whitespace-pre-wrap break-words text-left"><?= e($reply['content']) ?></p>
                        </div>
                    </div>
                </article>
            <?php endforeach; ?>
